...
public class PersonRepositoryImpl implements PersonRepositoryCustom {
	...
	public List<Person> findAllByFilter(PersonFilterDTO personFilter, Pageable pageable,
	...
	/*
	 * 
	 * This method uses CriteraBuilder to filter and sort data at query level. 
	 * this is an alternative to using native query
	 * 
	 * 
	@Override
	public List<Person> findAllByFilter(PersonFilterDTO personFilter, Pageable pageable,
		...
		if (personFilter.getFirstName() != null && !personFilter.getFirstName().isEmpty()) {
			String likeStr = "%".concat(personFilter.getFirstName()).concat("%");
			predicateList.add(criteriaBuilder.like(personRoot.get("firstName"), likeStr));
		}
		if (personFilter.getLastName() != null && !personFilter.getLastName().isEmpty()) {
			String likeStr = "%".concat(personFilter.getLastName()).concat("%");
		...
		if (personFilter.getOccupation() != null && !personFilter.getOccupation().isEmpty()) {
			String likeStr = "%".concat(personFilter.getOccupation()).concat("%");
			predicateList.add(criteriaBuilder.like(personRoot.join("occupation",JoinType.LEFT).get("name"), likeStr));
		}
		if (personFilter.getFavoriteFood() != null && !personFilter.getFavoriteFood().isEmpty()) {
			String likeStr = "%".concat(personFilter.getFavoriteFood()).concat("%");
			predicateList.add(criteriaBuilder.like(personRoot.join("favoriteFood",JoinType.LEFT).get("name").as(String.class), likeStr));
		}

		criteriaQuery.select(personRoot).where(predicateList.toArray(new Predicate[0]));

		if (sortOrder != null) {
			if (sortOrder.getDirection() == SortDirection.DESCENDING) {
				criteriaQuery.orderBy(criteriaBuilder.desc(personRoot.get(sortOrder.getSorted().getKey())));
			} else {
				criteriaQuery.orderBy(criteriaBuilder.asc(personRoot.get(sortOrder.getSorted().getKey())));
			}
		}
